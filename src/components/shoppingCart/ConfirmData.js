...
const ConfirmData = () => {
  ...
  const editData = () => {
    dispatch(
      actions.updateUserInfo(
        'all',
        userID === 'guest' ? guestInfo : loggedUser.information
      )
    );
  ...
  return (
    <div id="confirm-data-container" className="brand-font-family">
      ...
      {userID !== loggedUser.uid && userID !== 'guest' && (
        <Redirect to="/cart" />
      )}
      <UserInfo guest={userID === 'guest' ? true : false} />
      <div id="control-buttons">
        <Button variant="outline-danger" onClick={cancelConfirmData}>
          Salir
        </Button>
        <Button variant="outline-warning" onClick={editData}>
          Editar
        </Button>
        <Button variant="success" onClick={confirmData}>
          Confirmar
        </Button>
      </div>
    </div>
